...
{
    uint8_t color = get_register8(emu, BL) & 0x0f; // BLから色コードを取得（mov ebx, 15）
    uint8_t ch = get_register8(emu, AL);           // ALから表示する文字を取得（mov al, [esi]）
...
{
    uint8_t func = get_register8(emu, AH); // AHから関数番号を取得（mov ah, 0x0e）
    switch (func)
    {
    case 0x0e:

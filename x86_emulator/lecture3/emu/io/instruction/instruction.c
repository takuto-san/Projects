...
#include "emulator_func/emulator_function.h"
#include "io/io.h"

#include "modrm/modrm.h"
...
{
    uint16_t address = get_register32(emu, EDX) & 0xffff; // DX（0x03f8）
    uint8_t value = io_in8(address);                      // 標準入力（0x41='A'）
    set_register8(emu, AL, value);                        // AL（0x41='A'）
    emu->eip += 1;
}
...
{
    uint16_t address = get_register32(emu, EDX) & 0xffff; // DX（0x03f8）
    uint8_t value = get_register8(emu, AL);               // AL（0x41='A'）
    io_out8(address, value);                              // 標準出力（0x41='A'）
    emu->eip += 1;
}
